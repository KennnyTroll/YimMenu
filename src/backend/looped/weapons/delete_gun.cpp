...
namespace big
{
	...
	inline std::string get_network_id_string(int16_t net_id)
	{
		...
		if (!net_obj)
			return std::format("net_id {}", net_id);

		if (auto game_obj = net_obj->GetGameObject(); !game_obj || !game_obj->m_model_info)
			return std::format("net_id {} ({})", net_id, net_object_type_strs[net_obj->m_object_type]);
					...
					if (entity::raycast(&entity))
					{
						...
						else
						{
							...
							else
							{
								...
								if (auto ptr = (rage::CDynamicEntity*)g_pointers->m_gta.m_handle_to_ptr(entity))
								{
									...
									{										
										...
										mess += std::format("{}\t", get_network_id_string(netobj->m_object_id));
										mess += std::format("m_object_type :{} {}\t", (int)netobj->m_object_type, net_object_type_strs[netobj->m_object_type]);
										mess += std::format("m_object_id :{}\t", (int)netobj->m_object_id);

										if (auto owner_plyr = g_player_service->get_by_id((uint32_t)netobj->m_owner_id))
										mess += std::format("m_owner_id :{} {}\t", (int)netobj->m_owner_id, owner_plyr->get_name());
										else
										mess += std::format("m_owner_id :{}\t", (int)netobj->m_owner_id);
										...
										mess += std::format("m_control_id :{}\t", (int)netobj->m_control_id);
										...
										mess += std::format("m_next_owner_id :{}\t", (int)netobj->m_next_owner_id);

										mess += std::format("m_is_remote :{}\t", (int)netobj->m_is_remote);
										mess += std::format("m_wants_to_delete {}\t", (int)netobj->m_wants_to_delete);
										mess += std::format("m_object_type :{}\t", (int)netobj->m_should_not_be_delete);
