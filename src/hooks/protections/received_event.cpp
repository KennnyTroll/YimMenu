...
namespace big
{
	...
	void hooks::received_event(rage::netEventMgr* event_manager, CNetGamePlayer* source_player, CNetGamePlayer* target_player, uint16_t event_id, int event_index, int event_handled_bitset, int buffer_size, rage::datBitBuffer* buffer)
	{
		...
		case eNetworkEvents::REQUEST_CONTROL_EVENT:
		{
			...
			auto player = g_player_service->get_by_id(source_player->m_player_id);
			if (player &&  player->redirect_cage_object_Obj != -1) [[unlikely]]
			{
				CObject* cobj = reinterpret_cast<CObject*>(g_pointers->m_gta.m_handle_to_ptr(player->redirect_cage_object_Obj));				
				if (cobj && cobj->m_net_object->m_object_id == (int16_t)net_id) 
				{
					LOG(INFO) << std::format("===> RECEIVED EVENT: REQUEST_CONTROL_EVENT - object id {} => FROM {}",
					    (int)net_id,
					    player->get_name());
					entity::entity_info_mgr(*g_pointers->m_gta.m_network_object_mgr, (int)net_id);
		...
		{			
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (source_player->m_player_id != self::id)
			...
			if (count)
			{
				...
				if (plyr && plyr->recev_log) [[unlikely]]
				{
					...
					entity::entity_info_mgr(*g_pointers->m_gta.m_network_object_mgr, (int)object_net_id);

				}

				//if (plyr && (int)object_net_id == plyr->target_object_id && plyr->target_object_id != -1) [[unlikely]]
				//{
				//	LOG(INFO) << std::format("===> RECEIVED EVENT: GIVE_CONTROL_EVENT - object id {} => FROM {}", (int)object_net_id, plyr->get_name());
				//	entity::entity_info_mgr(*g_pointers->m_gta.m_network_object_mgr, (int)object_net_id);
				//}
